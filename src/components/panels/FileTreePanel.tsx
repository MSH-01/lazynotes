...
export function FileTreePanel({
      ...
      // TODOS TAB
        ...
        // Expand/collapse category or toggle todo
          ...
          if (
            ...
            todos.expandedCategories.has(selectedItem.fullPath || selectedItem.name)
          ) {
            actions.toggleExpandCategory(selectedItem.fullPath || selectedItem.name);
        ...
        // Toggle completion with x or space
          ...
          if (visualMode.active) {
            ...
            const todoIds = selection
              .filter((item): item is TodoListItem & { type: 'todo'; id: string } => item.type === 'todo' && 'id' in item)
              .map((item) => item.id);
            if (todoIds.length > 0) {
              actions.batchToggleTodoComplete(todoIds);
              actions.exitVisualMode();
            }
          } else if (selectedItem?.type === 'todo' && 'id' in selectedItem) {
  ...
  return (
    <Panel title={titleContent} isFocused={isFocused} flexGrow={1}>
      {activeTab === 'files' ? <FileTree maxHeight={maxHeight} /> : <TodoList maxHeight={maxHeight} />}
    </Panel>
  );
}
